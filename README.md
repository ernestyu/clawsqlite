...
- **Pure SQLite backend**
  ...
  - `articles_fts` FTS5 table for full‑text search over generated_title/tags/summary/body
...
- `generated_title` (LLM-generated knowledge title)
- `source_title` (source/archive title used for filenames and metadata)
- `summary`
- `tags`
- `key_claims`
- `entities`
...
- ~25% BM25 keywords for textual sanity checks (FTS over generated_title/tags/summary)
...
The knowledge app will parse these into `source_title` and markdown body.
...
- Call your scraper
- Extract source title + markdown body
...
- If a record with this `source_url` exists (including a soft-deleted record),
  ...
  - The knowledge app updates that record’s `source_title` / `generated_title` / `summary` / `tags` / `category` / `priority`
  - Keeps the same `id`
...
In strict mode, manual `--title` is a source-title hint for filenames and
metadata; the stored `generated_title`, tags, category, and content type must
be generated by the LLM. Tags must match
...
- `admin index rebuild --fts-table ... --fts-cols title,tags,summary` is a
  ...
  provided, it rebuilds only FTS columns that are DB-backed by the base table.
  In the knowledge schema, FTS `title` is populated from `articles.generated_title`.
  File consistency belongs under `admin fs`; application-level rebuild workflows
  should use the `knowledge` wrappers where available.
...
- `admin fs repair --json` recreates missing article Markdown files for DB rows:
  ...
  summaries fall back to DB summary/generated_title content. It does not rebuild indexes
...
```text
<id:06d>__<slugified-source-title>.md
```

- `id` comes from the `articles` table
- `slugified-source-title` is derived from `source_title`:
...
For CJK source titles, the filename is typically a pinyin‑based slug. If `pypinyin`
is not installed, it may fall back to `untitled`. This does not affect functionality:

- `source_title` and `generated_title` are stored in the DB
