...
If cleaned input content is already shorter than that target, the stored summary
is the cleaned content itself so short notes are not lossy paraphrases.
...
In strict mode, manual title/category/tags are hints only. The stored title,
tags, category, and content type must be generated by the LLM. Tags must match
...
`config_path`, `root`, `db`, `articles_dir`, `generation_quality`, and
