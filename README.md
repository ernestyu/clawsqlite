...
In strict mode, manual tags are hints only. The stored tags must be generated by
the LLM, and their count must match `[ingest].tag_count` (default `8`). The
stored category must be one of `[ingest].allowed_categories`; strict LLM ingest
stores the generated `content_type` as the category. Successful JSON output
includes `config_path`, `root`, `db`, `articles_dir`, `generation_quality`, and
`embedding_enabled` so Agents can audit where the record was written.
