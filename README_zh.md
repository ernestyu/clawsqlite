...
之前的名字是 “Clawkb”，现在已经全部统一为 `clawsqlite` / `clawsqlite knowledge`。
...
旧的 `clawkb` 入口已经从代码中移除，现在默认只提供 `clawsqlite`
这一套命令。Embedding / 抓取等配置建议通过外部环境变量
...
3. 环境变量：`CLAWKB_ROOT`（兼容旧配置）
4. 环境变量：`CLAWSQLITE_ROOT_FALLBACK` / `CLAWKB_ROOT_FALLBACK`
5. 默认：`<当前工作目录>/knowledge_data`

**数据库路径：**

- `--db` > `CLAWSQLITE_DB` > `CLAWKB_DB`（兼容）> `<root>/knowledge.sqlite3`

**Markdown 目录：**

- `--articles-dir` > `CLAWSQLITE_ARTICLES_DIR` > `CLAWKB_ARTICLES_DIR`（兼容）> `<root>/articles`
...
运行时，建议在外部环境中（例如 OpenClaw agent 配置）设置好这些
环境变量，然后直接调用 `clawsqlite knowledge ...`。本仓库不再包含
自动加载项目 `.env` 的入口脚本逻辑。
...
- 使用 `CLAWSQLITE_VEC_DIM`（或兼容的 `CLAWKB_VEC_DIM`）创建
...
- CLI：`--scrape-cmd`
- 环境变量：`CLAWSQLITE_SCRAPE_CMD`（推荐）或 `CLAWKB_SCRAPE_CMD`（兼容）
...
- `--scrape-cmd`：抓取命令（或用 `CLAWSQLITE_SCRAPE_CMD`/`CLAWKB_SCRAPE_CMD`）
